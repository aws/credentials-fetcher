...
To install the credentials-fetcher daemon:

```bash
# Build and install the daemon
brazil-build cf-install
```

### Generate Accurate Coverage Reports

To generate coverage reports that exclude proto files and other generated code:

```bash
# After running tests with Brazil
./scripts/update-coverage-report.sh
```
...
./scripts/generate-coverage-html.sh
```
