...
func TestGetMetadataFilePaths(t *testing.T) {
	...
	for _, file := range validFiles {
		...
		err := os.WriteFile(path, []byte("{}"), 0644) /* #nosec */
		require.NoError(t, err)
	}
...
func TestReadMetadataJSON(t *testing.T) {
	...
	t.Run("New Format", func(t *testing.T) {
		...
		require.NoError(t, err)
		err = os.WriteFile(newFormatPath, newFormatJSON, 0644) /* #nosec */
		require.NoError(t, err)
	...
	t.Run("Old Format", func(t *testing.T) {
		...
		require.NoError(t, err)
		err = os.WriteFile(oldFormatPath, oldFormatJSON, 0644) /* #nosec */
		require.NoError(t, err)
	...
	t.Run("Invalid JSON", func(t *testing.T) {
		...
		err = os.WriteFile(invalidPath, []byte("invalid json"), 0644) /* #nosec */
		require.NoError(t, err)
...
func TestWriteMetaDataJSON(t *testing.T) {
	...
	t.Run("Directory Creation Error", func(t *testing.T) {
		...
		err = os.WriteFile(conflictPath, []byte("test"), 0644) /* #nosec */
		require.NoError(t, err)
