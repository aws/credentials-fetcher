...
void insert_records_to_cache( creds_fetcher::CF_cache& cf_cache,
                              std::list<creds_fetcher::krb_ticket_info> krb_ticket_infos )
{
    cf_cache.sql =
        "INSERT INTO krb_cache (lease_id, krb_file_path, service_account_name, domain_name) "
    ...
    cf_cache.read_connection =
        sqlite3_exec( cf_cache.db, cf_cache.sql.c_str(), callback, 0, &cf_cache.errMsg );
...
void read_records_from_cache( creds_fetcher::CF_cache& cf_cache )
{
    ...
    /* Execute SQL statement */
    cf_cache.read_connection =
        sqlite3_prepare_v2( cf_cache.db, cf_cache.sql.c_str(), -1, &stmt, 0 );
    ...
    while ( sqlite3_step( stmt ) == SQLITE_ROW )
    {
        ...
        // krb_ticket_info.krb_file_path = std::string(reinterpret_cast<const char
        // *>(sqlite3_column_text( stmt, 1 )));
        // krb_ticket_info.service_account_name =  std::string(reinterpret_cast<const char
        // *>(sqlite3_column_text( stmt, 2 ))); krb_ticket_info.domain_name =
