...
std::vector<creds_fetcher::krb_ticket_info *> read_meta_data_json(std::string lease_id,
    ...
    std::vector < creds_fetcher::krb_ticket_info * > krb_ticket_info_list;
    try {
        ...
        fprintf(stderr, SD_CRIT
        "meta data file is not properly formatted" );
        return krb_ticket_info_list;
    }
 ...
 * {
    "krb_ticket_info": [
        {
            "krb_file_path": "\/usr\/share\/credentials-fetcher\/krbdir\/3e1ff0bb9f966192c440\/ccname_WebApp01_OPMsbZ",
...
int write_meta_data_json(std::list<creds_fetcher::krb_ticket_info *> krb_ticket_info_list,
    ...
    try {
        ...
        fprintf(stderr, SD_CRIT
        "failed to write meta data file" );
        return -1;
    }
    return 0;
}